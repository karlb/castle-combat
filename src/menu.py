...
class NewGameState(WidgetState):
    def __init__(self):
        ...
        def start_game(total_players, local_players, ai_players):
            ...
            options = dict(difficulty=self.difficulty.value.lower().replace(" ", "_"))
            ser = server.Server(self.selected_map(options), total_players)
            ser.run(local_players, ai_players)

        def on_total_change():
            self.local_players.value = min(
                self.local_players.value, self.total_players.value
            )
            self.local_players.choices = list(range(self.total_players.value, 0, -1))
            from map import maps

            self.map.choices = [
                m.title for m in maps if self.total_players.value in m.allowed_players
            ]
            on_map_change()
        ...
        def on_map_change():
            ...
            self.selected_map = [
                m
                for m in maps
                if m.title == self.map.value
                and self.total_players.value in m.allowed_players
            ][0]
        ...
        self.total_players = SpinBox(
            "Total Players",
            (50, 360),
            (4, 3, 2, 1),
            default=conf.total_players,
            on_change=on_total_change,
        )
        self.local_players = SpinBox(
            "Local Players", (50, 400), (4, 3, 2, 1), default=conf.local_players
        )
        if common.debug:
            self.ai_players = SpinBox(
                "AI Players", (50, 440), (2, 1, 0), conf.ai_players
            )
            Button(
                "Start Game",
                (None, 250),
            ...
            Button(
                ...
                (None, 250),
                lambda: start_game(
                    self.total_players.value, self.local_players.value, 0
                ),
            )

        self.map = SpinBox("Map", (470, 360), ("dummy",), on_change=on_map_change)
        self.difficulty = SpinBox(
            "Difficulty",
            (470, 400),
...
class JoinGameState(WidgetState):
    def __init__(self):
        ...
        Button(
            "Join Game",
            (None, 250),
            lambda: join(self.server.value, self.local_players.value),
        )
...
class ConfigState(WidgetState):
    def __init__(self):
        ...
        for i in (0, 1):
            ...
            KeyConfigButton(
                "Right", (50 + i * 500, top + line_height * 4), self.right[i]
            )
            KeyConfigButton(
                "Button 1", (50 + i * 500, top + line_height * 5), self.button[i][0]
            )
            KeyConfigButton(
                "Button 2", (50 + i * 500, top + line_height * 6), self.button[i][1]
            )
        ...
        self.sound = SpinBox(
            ...
            (265, -20),
            ("On", "Off"),
            default=bool_to_on_off[conf.sound],
            on_change=sound_change,
        )
        self.fullscreen = SpinBox(
            "Fullscreen",
            (485, -20),
            ("On", "Off"),
            default=bool_to_on_off[conf.fullscreen],
        )
