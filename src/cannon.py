...
class Shot:
    ...
    def __init__(self, cannon, target):
        ...
        self.time_to_target = (
            math.hypot(self.origin[0] - target[0], self.origin[1] - target[1]) * 7
        )
        sound.cannon.play()
    ...
    def get_pos(self):
        return list(
            map(
                lambda o, t: o * (1 - self.ratio) + t * self.ratio,
                self.origin,
                self.target,
            )
        )
...
class Cannon(common.AutoReloader):
    ...
    def blit(self):
        if self.hitpoints <= 0:
            return
        cannon_center = [
            (self.pos[i] + (2.0 + self.type) / 2) * common.block_size for i in range(2)
        ]
